...
class FeatureElicitator():
    ...
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            cmd_rad = (np.transpose(self.curr_pos) + np.diag(self.cmd) * self.delta_T)[0]
            self.joints_deg_pub.publish(Float64MultiArray(data=180 * cmd_rad/np.pi))
    ...
    def register_callbacks(self):
        ...
        """
        self.joints_deg_pub = rospy.Publisher("/siemens_demo/joint_cmd", Float64MultiArray, queue_size=10)
        self.is_intervene_pub = rospy.Publisher("/is_intervene", Bool, queue_size=10)
        # Create subscriber to joint_angles.
        rospy.Subscriber('/kinova/current_joint_state',Float64MultiArray, self.joint_angles_callback, queue_size=1)
        # Create subscriber to joint torques
        rospy.Subscriber('/kinova/current_joint_torques',Float64MultiArray, self.joint_torques_callback, queue_size=1)
