...
class TrajoptPlanner(object):
	...
	def learned_feature_costs(self, waypt):
		...
		for i, feature in enumerate(self.environment.learned_features):
			...
			feature_values.append(self.interpolate_features(
				curr_waypt, prev_waypt, feat_idx))
		# calculate the cost
	...
	def learned_feature_cost_derivatives(self, waypt):
		...
		for i, feature in enumerate(self.environment.learned_features):
			...
			for step in range(NUM_STEPS):
				...
				z = self.environment.feature_func_list[feat_idx](
					self.environment.raw_features(inter_waypt).float(), torchify=True)
				feat_val = feat_val + z
			y = feat_val / torch.tensor(float(NUM_STEPS), requires_grad=True)
			y = y * torch.tensor(self.environment.weights[-n_learned + i:],
			                     requires_grad=True) * torch.norm(x[7:] - x[:7])
			y.backward()
	...
	def trajOpt(self, start, goal, goal_pose, traj_seed=None):
		...
		if traj_seed is None:
			...
			for count in range(self.num_waypts):
				init_waypts[count, :] = start + count / \
					(self.num_waypts - 1.0) * (goal - start)
		else:
		...
		if goal_pose is not None:
			...
			import pdb
			pdb.set_trace()
			xyz_target = goal_pose[0:3].tolist()
			quat_target = goal_pose[[-1, 3, 4, 5]].tolist()  # [1, 0, 0, 0]  #wxyz
			constraint = [
				{
					"type": "pose",
		...
		for t in range(1, self.num_waypts):
			if 'coffee' in self.environment.feature_list:
				prob.AddCost(self.coffee_cost, [
				             (t - 1, j) for j in range(7)] + [(t, j) for j in range(7)], "coffee%i" % t)
			if 'table' in self.environment.feature_list:
				prob.AddCost(self.table_cost, [
				             (t - 1, j) for j in range(7)] + [(t, j) for j in range(7)], "table%i" % t)
			if 'laptop' in self.environment.feature_list:
				prob.AddCost(self.laptop_cost, [
				             (t - 1, j) for j in range(7)] + [(t, j) for j in range(7)], "laptop%i" % t)
			if 'origin' in self.environment.feature_list:
				prob.AddCost(self.origin_cost, [
				             (t - 1, j) for j in range(7)] + [(t, j) for j in range(7)], "origin%i" % t)
			if 'human' in self.environment.feature_list:
				prob.AddCost(self.human_cost, [
				             (t - 1, j) for j in range(7)] + [(t, j) for j in range(7)], "human%i" % t)
			if 'efficiency' in self.environment.feature_list:
				prob.AddCost(self.efficiency_cost, [
				             (t - 1, j) for j in range(7)] + [(t, j) for j in range(7)], "efficiency%i" % t)
			if 'learned_feature' in self.environment.feature_list:
				prob.AddErrorCost(self.learned_feature_costs, self.learned_feature_cost_derivatives, [(
					t - 1, j) for j in range(7)] + [(t, j) for j in range(7)], "ABS", "learned_features%i" % t)
